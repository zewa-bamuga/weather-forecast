...
class User(Base):
    ...
    email: orm.Mapped[str] = orm.mapped_column(sa.String, unique=True, nullable=True)
    ...
    tokens: orm.Mapped[list["Token"]] = orm.relationship("Token", back_populates="user")
    search_histories: orm.Mapped[list["SearchHistory"]] = orm.relationship("SearchHistory", back_populates="user")
...
class SearchHistory(Base):
    ...
    city: orm.Mapped[str]
    last_searched_at: orm.Mapped[datetime.datetime] = orm.mapped_column(sa.DateTime(timezone=True),
                                                                        server_default=func.now())

    user: orm.Mapped["User"] = orm.relationship("User", back_populates="search_histories")
...
class Token(Base):
    ...
    user: orm.Mapped["User"] = orm.relationship("User", back_populates="tokens")
